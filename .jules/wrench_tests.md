...
- **Azure Identity Provider:** The Azure Identity provider (`collector/internal/providers/universal_rest/azure_identity.go`) uses a global singleton cache (`globalAzureCache`) for caching tokens. To prevent test pollution, tests exercising this flow must reset the cache using a `defer` block after execution.
