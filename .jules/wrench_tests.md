...
- **Test Pollution**: When writing table-driven tests in Go that involve mutable objects passed as pointers (like `*http.Request` given to functions that modify it), instantiate the object inside the subtest loop rather than reusing a single instance outside the loop to prevent cross-test pollution and state mutation.
