package compliance.controls.vpm_10_test

import rego.v1
import data.compliance.controls.vpm_10

# Helper to build mock input with a single Dataset source
mock_input(evidence_id, issues) = {
	"findings": {
		evidence_id: {
			"aikido_src": {
				"raw_data": issues,
...
test_empty_issues if {
	vpm_10.compliant with input as mock_input("EVID-VPM-10", [])
}
...
test_all_closed_issues if {
	...
	vpm_10.compliant with input as mock_input("EVID-VPM-10", issues)
}
...
test_open_critical_fails if {
	...
	not vpm_10.compliant with input as mock_input("EVID-VPM-10", issues)
}
...
test_open_high_fails if {
	...
	not vpm_10.compliant with input as mock_input("EVID-RSK-04", issues)
}

# Test: Cross-Dataset evaluation with critical in secondary Dataset should fail
