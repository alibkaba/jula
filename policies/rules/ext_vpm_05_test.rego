package compliance.controls.vpm_05_test

import rego.v1
import data.compliance.controls.vpm_05
...
test_empty_issues if {
	vpm_05.compliant with input as mock_input("EVID-VPM-10", [])
}
...
test_all_closed_issues if {
	...
	vpm_05.compliant with input as mock_input("EVID-VPM-10", issues)
}
...
test_medium_low_severity_passes if {
	...
	vpm_05.compliant with input as mock_input("EVID-VPM-05", issues)
}
...
test_open_critical_fails if {
	...
	not vpm_05.compliant with input as mock_input("EVID-VPM-10", issues)
}
...
test_open_high_fails if {
	...
	not vpm_05.compliant with input as mock_input("EVID-MNT-03", issues)
}
