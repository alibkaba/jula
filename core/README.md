...
Core ships two standalone command-line tools for evidence signing and verification.
...
Core CLI tools are released via the `Pipeline: Release Artifacts` workflow when a `core-v*` tag is published. Binaries are built for macOS ARM64, Linux AMD64, and Windows AMD64 with SLSA provenance attestation.
